# Anthropic Admin API Cookbook: Usage & Cost Tracking
...
1. **An Anthropic Admin API Key** (format: `sk-ant-admin...`)
   - Get it from the [Claude Console](https://console.anthropic.com/settings/admin-keys)
2. **Python 3.8+** with the required packages
...
Create a secure wrapper for the Anthropic Admin API endpoints:
...
First, verify that your API credentials are working correctly:
...
## Step 2: Retrieve Daily Usage Data

The Messages Usage API provides token consumption in **time buckets** - fixed intervals containing aggregated usage. Let's create a function to get usage data for the last N days:
...
Now, let's process and display the usage data to understand key metrics:
...
## Step 5: Group Usage by Model

The API supports grouping data by various dimensions. Let's analyze usage by model:
...
## Step 6: Filter Usage Data

You can filter usage data by specific criteria like model or service tier:
...
## Step 7: Handle Pagination for Large Datasets

For large datasets, you need to handle pagination properly:
...
Export your usage data to CSV for external analysis:
...
def export_usage_to_csv(client, output_file="usage_data.csv", days_back=30):
    ...
    try:
        ...
        while page_count < max_pages:
            ...
            # Process this page's data
            for bucket in response.get("data", []):
                bucket_start = bucket["starting_at"][:10]
