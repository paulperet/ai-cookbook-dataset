# Building a Crop Tool for Enhanced Claude Image Analysis

When Claude analyzes an image, it processes the entire image at once. For detailed tasks—like reading small text, comparing similar values in charts, or examining fine details—this can be limiting. This guide shows you how to build a crop tool that lets Claude "zoom in" on specific regions of interest for more precise analysis.

## When to Use a Crop Tool

A crop tool is particularly useful for:
- **Charts and graphs**: Comparing bars/lines with close values, reading axis labels
...
```bash
pip install anthropic pillow datasets
```
...
Let's test the crop tool manually by cropping the legend area (top-right of most charts):
...
This works because Claude can see the full image first, identify regions that need closer inspection, and iteratively zoom in. The approach can be implemented either directly with the Anthropic API or using the more structured Claude Agent SDK.