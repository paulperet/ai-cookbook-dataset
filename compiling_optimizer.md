...
become the bottleneck in training performance for large models. In this
recipe, we will apply `torch.compile` to the optimizer to observe the
...
For this example, we\'ll use a simple sequence of linear layers. Since
we are only benchmarking the optimizer, the choice of model doesn\'t
matter because optimizer performance is a function of the number of
parameters.
...
In this example, we\'ll use the Adam optimizer and create a helper
function to wrap the step() in `torch.compile()`.
...
`torch.compile` is only supported on cuda devices with compute
capability \>= 7.0
::::
...
\* For an in-depth technical overview, see [Compiling the optimizer with
